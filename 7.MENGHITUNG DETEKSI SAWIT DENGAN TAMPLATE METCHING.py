...
mobil = cv2.imread('gambar/mobil2.jpg')
mobil = cv2.cvtColor(mobil, cv2.COLOR_BGR2RGB)

# Panggil dan konversi warna agar sesuai dengan Matplotlib
parkir = cv2.imread('gambar/mobilbanyak.jpg')
parkir = cv2.cvtColor(parkir, cv2.COLOR_BGR2RGB)

plt.subplot(121),plt.imshow(mobil), plt.title('mobil')
plt.subplot(122),plt.imshow(parkir), plt.title('mobilbanyak')
plt.show()


# Membaca gambar utuh untuk dicari
img_rgb = cv2.imread('gambar/mobilbanyak.jpg')
img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)

# Membaca template
template = cv2.imread('gambar/mobil2.jpg', 0)
