...
fadil = cv2.imread('gambar/fadil.jpg')
fadil = cv2.cvtColor(fadil, cv2.COLOR_BGR2RGB) # simpan dengan nama yang sama ditumpuk

# panggil dan konversi warna agar sesuai dengan Matplotlib
wirus = cv2.imread('gambar/wirus.jpg')
wirus = cv2.cvtColor(wirus, cv2.COLOR_BGR2RGB) 
plt.subplot(121),plt.imshow(fadil), plt.title('fadil')
plt.subplot(122),plt.imshow(wirus), plt.title('wirus')
plt.show()
import cv2
import numpy as np
from matplotlib import pyplot as plt
img = cv2.imread('gambar/wirus.jpg',0)
img2 = img.copy()
template = cv2.imread('gambar/fadil.jpg',0)
